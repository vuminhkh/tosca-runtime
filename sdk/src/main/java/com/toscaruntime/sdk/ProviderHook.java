...
public interface ProviderHook {
     ...
     * @return all nodes that the provider processed workflow for and so will not be processed by the workflow engine
     */
    ProviderWorkflowProcessingResult postConstructInstallWorkflow(Map<String, Root> nodeInstances, Set<tosca.relationships.Root> relationshipInstances, Map<Root, InstallLifeCycleTasks> nodesLifeCycles);
     ...
     * @param nodeInstances         all nodes impacted by the workflow
     * @param relationshipInstances all relationship instances impacted by the workflow
     * @param nodesLifeCycles       install life cycle tasks of nodes
     * @return all nodes that the provider processed workflow for and so will not be processed by the workflow engine
     */
