...
object AgentsSteps extends MustMatchers {
  ...
  def cancelDeployment(deploymentId: String, force: Boolean = false) = AgentCommand.cancelExecution(Context.client, deploymentId, force)

  def pauseDeployment(deploymentId: String, force: Boolean = false) = AgentCommand.stopExecution(Context.client, deploymentId, force)

  def scale(deploymentId: String, nodeName: String, newInstancesCount: Int) = {
    AgentCommand.scaleExecution(Context.client, deploymentId, nodeName, newInstancesCount)
  ...
  def executeNodeOperation(deploymentId: String, nodeName: Option[String], operationName: String, instanceId: Option[String] = None, interfaceName: Option[String] = None, inputs: Map[String, String] = Map.empty, transient: Boolean = false) = {
    AgentCommand.executeNodeOperation(Context.client, deploymentId, nodeName, instanceId, interfaceName, operationName, inputs, transient)
    Await.result(Context.client.waitForRunningExecutionToEnd(deploymentId), 15 minutes)
  }
  ...
  def executeRelationshipOperation(deploymentId: String, sourceName: Option[String], targetName: Option[String], relationshipType: String, operationName: String, sourceId: Option[String] = None, targetId: Option[String] = None, interfaceName: Option[String] = None, inputs: Map[String, String] = Map.empty, transient: Boolean = false) = {
    AgentCommand.executeRelationshipOperation(Context.client, deploymentId, sourceName, sourceId, targetName, targetId, relationshipType, interfaceName, operationName, inputs, transient)
    Await.result(Context.client.waitForRunningExecutionToEnd(deploymentId), 15 minutes)
  }
  ...
  def launchDeployment(deploymentId: String) = {
    Await.result(AgentCommand.deploy(Context.client, deploymentId)._2, 20 minutes)
  }

  def resumeDeployment(deploymentId: String) = {
    AgentCommand.resumeExecution(Context.client, deploymentId)
  }

  def launchUndeployment(deploymentId: String) = {
    Await.result(AgentCommand.undeploy(Context.client, deploymentId, force = false, teardown = false)._2, 20 minutes)
  }

  def launchInstallWorkflow(deploymentId: String) = {
    AgentCommand.launchInstallWorkflow(Context.client, deploymentId)
  ...
  def launchUninstallWorkflow(deploymentId: String) = {
    AgentCommand.launchUninstallWorkflow(Context.client, deploymentId, force = false)
    Await.result(Context.client.waitForRunningExecutionToEnd(deploymentId), 20 minutes)
  }

  def createAgent(deploymentId: String) = {
    AgentCommand.createAgent(Context.client, deploymentId)
  }

  def listAgents() = {
    AgentCommand.listAgents(Context.client)
  }

  def startAgent(deploymentId: String) = {
    AgentCommand.startAgent(Context.client, deploymentId)
  }

  def restartAgent(deploymentId: String) = {
    AgentCommand.restartAgent(Context.client, deploymentId)
  }

  def assertDeploymentHasNode(deploymentId: String, nodeName: String, instanceCount: Int, state: Int = NODE_STARTED) = {
    val nodesFound = AgentCommand.getNodesDetails(Context.client, deploymentId).filter(nodeDetails => nodeDetails.head == nodeName)
  ...
  def assertDeploymentHasRelationship(deploymentId: String, sourceName: String, targetName: String, instanceCount: Int, state: Int = RELATIONSHIP_ESTABLISHED) = {
    val relationsFound = AgentCommand.getRelationshipsDetails(Context.client, deploymentId).filter { relationshipDetails =>
  ...
  def assertDeploymentHasOutput(deploymentId: String, key: String) = {
    val found = AgentCommand.getOutputsDetails(Context.client, deploymentId).filter(outputEntry => outputEntry.head == key && outputEntry.last.nonEmpty)
  ...
  def stopAgent(deploymentId: String) = {
    AgentCommand.stopAgent(Context.client, deploymentId)
  }

  def deleteAgent(deploymentId: String) = {
    AgentCommand.deleteAgent(Context.client, deploymentId)
  }
