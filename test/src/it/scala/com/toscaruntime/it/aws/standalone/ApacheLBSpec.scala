package com.toscaruntime.it.aws.standalone
...
class ApacheLBSpec extends AbstractSpec with MustMatchers {
  ...
  info("Test deployment of a topology apache load balancer with aws in mode masterless")
  ...
  feature("Deployment of a load balanced web app") {
    scenario("Standard deployment") {
      ...
      And("A deployment image has been created for the tomcat with apache load balancer aws topology")
      createDeploymentImage("apache-lb", awsProvider) must be(true)
