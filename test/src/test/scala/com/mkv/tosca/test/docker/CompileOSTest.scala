...
object CompileOSTest {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val compiled = Compiler.compile(wordpressTopology, List(apachePath, mysqlPath, phpPath, wordpressPath), openstackPath, outputDeployment)
    if (compiled) {
      ...
      Deployer.deploy(outputDeployment, inputs, providerProperties, true)
    }
  }
}
