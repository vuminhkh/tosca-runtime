...
import com.toscaruntime.exception.{AgentNotRunningException, BadClientConfigurationException, DaemonResourcesNotFoundException}
  ...
  def createAgentImage(deploymentId: String, bootstrap: Boolean, recipePath: Path, inputsPath: Option[Path], providerConfigPath: Path, bootstrapContext: Map[String, Any]) = {
    ...
    if (!Files.exists(providerConfigPath.resolve("provider.conf"))) {
      if (!Files.exists(providerConfigPath.resolve("auto_generated_provider.conf"))) {
        throw new BadClientConfigurationException(s"Provider configuration (provider.conf or auto_generated_provider.conf) is not found at $providerConfigPath")
      } else {
