...
object Deployer {
   ...
   */
  def deploy(deploymentRecipeFolder: Path, inputFile: Option[Path], providerProperties: Map[String, String], bootstrap: Boolean): Deployment = {
    val inputs = DeployerUtil.loadInputs(inputFile)
    deploy(deploymentRecipeFolder, inputs, providerProperties, bootstrap)
  }
